    ...
    internal static class Util
    {
        ...
        private static string ConstructLinqFilterExpression(JQGrid grid, SearchArguments args)
        {
            ...
            if (column.DataType == typeof(DateTime))
            {
                ...
                // Edited: Sergei Lobachov
                // to include current date
                if (args.SearchOperation == SearchOperation.IsLessThan)
                {
                    time = time.AddDays(1);
                }
        ...
        public static string GetWhereClause(JQGrid grid, NameValueCollection queryString)
        {
            ...
            foreach (JQGridColumn column in grid.Columns)
            {
                string searchString = queryString[column.DataField];
                if (!string.IsNullOrEmpty(searchString))
                {
                    string searchColumn = column.DataField;
                    SearchOperation searchOperation = column.SearchToolBarOperation;

                    // Sergei: added to allow searching on custom formatted columns 
                    // works only for autocomplete with BeginsWith searchoption
                    if (column.Formatter != null)
                    {
                        var cust = column.Formatter as CustomFormatter;
                        if (cust != null)
                        {
                            if (!String.IsNullOrEmpty(cust.SetAttributesFunction))
                            {
                                if (cust.SetAttributesFunction == "colHideAttrJqGrid"
                                    && searchOperation == SearchOperation.BeginsWith)
                                {
                                    searchString = "0";
                                    searchOperation = SearchOperation.IsGreaterThan;
                                }
                            }
                        }
                    }
