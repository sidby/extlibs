namespace Trirand.Web.Mvc
{
    ...
    internal class JQDatePickerRenderer
    {
        ...
        private string GetStandaloneJavascript()
        {
            ...
            // apend fromat
            builder.Append("<style type='text/css'>body .ui-datepicker { z-index: 2000 }</style>");
        ...
        private string GetStartupOptions()
        {
            ...
            // added hack to trigger search toolbar event after date is selected
            // since we dont know gridview id on which was attached this datepicker
            // in 
            //string dateSelectedFunctionName = this._model.ID + "DateSelectedTrigger";
            if (!String.IsNullOrEmpty(this._model.OnSelectedCallbackFunction))
                sb.Append(",onSelect: function(dateText, inst) { if(typeof(" + this._model.OnSelectedCallbackFunction + ") === 'function') { " + this._model.OnSelectedCallbackFunction + "(dateText, inst); } }");
