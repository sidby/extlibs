namespace Trirand.Web.Mvc
{
    ...
    internal class JQGridRenderer
    {
        ...
        private string GetStartupOptions(JQGrid grid, bool subGrid)
        {
            ...
            builder.AppendFormat(",scrollrows: {0}", grid.ScrollToSelectedRow.ToString().ToLower());
            ...
            if (grid.LoadOnce)
            {
                builder.Append(",loadonce: true");
                builder.Append(",ignoreCase: true");
            }
            ...
            builder.AppendFormat(",viewRowDialogOptions: {0}", this.GetViewRowOptions(grid));
            if (grid.TreeGridSettings.Enabled)
            {
            ...
            {
                builder.Append(",hiddengrid:true");
            }
            if (!grid.AppearanceSettings.ShowHideGridCaptionButton)
            {
                builder.Append(",hidegrid:false");
            }
        ...
        private string GetToolBarOptions(JQGrid grid, bool subGrid, string pagerSelectorID)
        {
            ...
            if (!subGrid)
            {
                builder.AppendFormat(".navGrid('#{0}',{1},{2},{3},{4},{5},{6} )", new object[] { grid.ID + "_pager", new JavaScriptSerializer().Serialize(bar), string.Format("jQuery('#{0}').getGridParam('editDialogOptions')", grid.ID), string.Format("jQuery('#{0}').getGridParam('addDialogOptions')", grid.ID), string.Format("jQuery('#{0}').getGridParam('delDialogOptions')", grid.ID), string.Format("jQuery('#{0}').getGridParam('searchDialogOptions')", grid.ID), string.Format("jQuery('#{0}').getGridParam('viewRowDialogOptions')", grid.ID) });
            }
            else
            {
                builder.AppendFormat(".navGrid('#' + pager_id,{0},{1},{2},{3},{4} )", new object[] { new JavaScriptSerializer().Serialize(bar), "jQuery('#' + subgrid_table_id).getGridParam('editDialogOptions')", "jQuery('#' + subgrid_table_id).getGridParam('addDialogOptions')", "jQuery('#' + subgrid_table_id).getGridParam('delDialogOptions')", "jQuery('#' + subgrid_table_id).getGridParam('searchDialogOptions')", "jQuery('#' + subgrid_table_id).getGridParam('viewRowDialogOptions')" });
            }
